...
MobileNet = GetNeuralNetworkModel('MobileNet',(32,32,3),10, quantization = False, aging_active=activation_aging)
...
wgt_dir= ('../Trained_Weights_cifar/MobileNet/weights.data')


#df = QuantizationEffect('MobileNet',test_ds,wgt_dir,(32,32,3),10,batch_size)
#
#
#quantizacion=pd.DataFrame(df)
#
#quantizacion.to_excel('quantizacion.xlsx', sheet_name='fichero_707', index=False)
#save_obj(df,'Data/Quantization/MobileNet_cifar/Quantization_MobileNet_cifar')

MobileNet.load_weights(wgt_dir)

score = MobileNet.evaluate(test_ds, verbose=1)
#
...
# #
# #
# score = MobileNet.evaluate(train_ds, verbose=1)
# #
...
for i in range(7):
    ...
    loss, acc = CheckAccuracyAndLoss('MobileNet', test_ds, wgt_dir, output_shape=10, input_shape=(32, 32, 3),
...
buf_cero.to_excel('MobileNet_Cifar_all_v_all_exp.xlsx', sheet_name='fichero_707', index=False)

# buf_cero = pd.concat([Voltajes,Acc_MobileNet,Acc_DenseNet,], axis=1, join='outer')
