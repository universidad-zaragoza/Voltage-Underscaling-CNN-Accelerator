...
# Inception = GetNeuralNetworkModel('Inception',(150,150,3),10, quantization = False, aging_active=False)
#
#
# # In[8]:
#
...
#
# score = Inception.evaluate(test_ds, verbose=1)
#
# print('Test loss:', score[0])
# print('Test accuracy:', score[1])

#
# score = Inception.evaluate(train_ds, verbose=1)
#
# print('Test loss:', score[0])
# print('Test accuracy:', score[1])

wgt_dir= ('../Trained_Weights_cifar/Inception/weights.data')
...
# #save_obj(df,'Data/Quantization/Inception_cifar/Quantization_Inception_cifar')
#
#
# # score = Inception.evaluate(train_ds, verbose=1)
# #
# # print('Test loss:', score[0])
# # print('Test accuracy:', score[1])
# print(str()+' Quantización completada Inception_Cifar: ', datetime.now().strftime("%H:%M:%S"))


# Inception.load_weights(wgt_dir)


#
print('Ahora si estoy en el que debe ser')
...
loss='sparse_categorical_crossentropy'
optimizer = Adam(0.0001)
WeightQuantization(model=Inception, frac_bits=11, int_bits=12)
...
for i in range(1):
    ...
    loss, acc = CheckAccuracyAndLoss('Inception', test_ds, wgt_dir, output_shape=10, input_shape=(150, 150, 3),
                                     act_frac_size=7, act_int_size=8, wgt_frac_size=11, wgt_int_size=12,
                                     batch_size=1, verbose=0, aging_active=True, weights_faults=False,
...
for i in range(1):
    ...
    loss, acc = CheckAccuracyAndLoss('Inception', test_ds, wgt_dir, output_shape=10, input_shape=(150, 150, 3),
                                     act_frac_size=8, act_int_size=7, wgt_frac_size=11, wgt_int_size=12,
                                     batch_size=1, verbose=0, aging_active=True, weights_faults=False,
                                     faulty_addresses=locs, masked_faults=error_mask)
    Accs_F_P.append(acc)
    fin = time.time()
...
# buf_cero.to_excel('Inception_Cifar_all_v_all_exp.xlsx', sheet_name='fichero_707', index=False)
...
buf_cero.to_excel('Inception_Cifar_base_F_P_quantiza_extend.xlsx', sheet_name='fichero_707', index=False)
