...
def GetNeuralNetworkModel(architecture, input_shape, output_shape, faulty_addresses = [],
	...
	if architecture=='AlexNet':
		...
		def res_identity(x, filters, i):
			...
			x = Conv2D(f1, kernel_size=(1, 1), strides=(1, 1), padding='valid', kernel_regularizer=tf.keras.regularizers.l2(0.001),name='Conv1_' + str(i))(x)
			x = BatchNormalization()(x, training=False)
			x = Activation(tf.keras.activations.relu)(x)
			x = AddCustomLayers(x, include_aging=True, aging_active=aging_active[4], include_quantization=False)

			x = Conv2D(f1, kernel_size=(3, 3), strides=(1, 1), padding='same', kernel_regularizer=tf.keras.regularizers.l2(0.001),name='Conv2_' + str(i))(x)
			x = BatchNormalization()(x, training=False)
			x = Activation(tf.keras.activations.relu)(x)
			x = AddCustomLayers(x, include_aging=True, aging_active=aging_active[5], include_quantization=False)

			x = Conv2D(f2, kernel_size=(1, 1), strides=(1, 1), padding='valid', kernel_regularizer=tf.keras.regularizers.l2(0.001),name='Conv3_' + str(i))(x)
			x = BatchNormalization()(x, training=False)
			x = Activation(tf.keras.activations.relu)(x)
			x = AddCustomLayers(x, include_aging=True, aging_active=aging_active[6], include_quantization=False)
			...
			x = tf.keras.layers.Add()([x, x_skip])
			x = Activation(tf.keras.activations.relu)(x)
			AddCustomLayers(x, include_aging=True, aging_active=aging_active[7], include_quantization=False)
		...
		def res_conv(x, s, filters, i):
			...
			x = Conv2D(f1, kernel_size=(1, 1), strides=(s, s), padding='valid', kernel_regularizer=tf.keras.regularizers.l2(0.001),name='Conv4_' + str(i))(x)
			x = BatchNormalization()(x, training=False)
			x = Activation(tf.keras.activations.relu)(x)
			x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[8])

			x = Conv2D(f1, kernel_size=(3, 3), strides=(1, 1), padding='same', kernel_regularizer=tf.keras.regularizers.l2(0.001),name='Conv5_' + str(i))(x)
			x = BatchNormalization()(x, training=False)
			x = Activation(tf.keras.activations.relu)(x)
			x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[9])

			x = Conv2D(f2, kernel_size=(1, 1), strides=(1, 1), padding='valid', kernel_regularizer=tf.keras.regularizers.l2(0.001),name='Conv6_' + str(i))(x)
			x = BatchNormalization()(x, training=False)

			x_skip = Conv2D(f2, kernel_size=(1, 1), strides=(s, s), padding='valid', kernel_regularizer=tf.keras.regularizers.l2(0.001),name='Conv7_' + str(i))(x_skip)
			x_skip = BatchNormalization()(x_skip, training=False)

			x = tf.keras.layers.Add()([x, x_skip])
			x = Activation(tf.keras.activations.relu)(x)
			AddCustomLayers(x, include_aging=True, aging_active=aging_active[10])
		...
		x = Conv2D(64, kernel_size=(7, 7), strides=(2, 2),name='Conv8_' + str(i))(x)
		x = BatchNormalization()(x, training=False)
		x = Activation(tf.keras.activations.relu)(x)
		x = AddCustomLayers(x, include_aging=True, aging_active=aging_active[12])
		x = MaxPool2D((3, 3), strides=(2, 2))(x)

		x = res_conv(x, s=1, filters=(64, 256), i = i+1)
		i = i+1
		x = res_identity(x, filters=(64, 256), i = i+1)
		i = i+1
        
		x = res_identity(x, filters=(64, 256), i = i+1)
		i = i+1
        
		x = res_conv(x, s=2, filters=(128, 512), i = i+1)
		i = i+1
        
		x = res_identity(x, filters=(128, 512), i = i+1)
		i = i+1
		...
		x = res_conv(x, s=2, filters=(256, 1024), i = i+1)
		i = i+1
        
		x = res_identity(x, filters=(256, 1024), i = i+1)
		i = i+1
		...
		x = res_conv(x, s=2, filters=(512, 2048), i = i+1)
		i = i+1
        
		x = res_identity(x, filters=(512, 2048), i = i+1)
		i = i+1
		...
		x = Conv2D(filters=6, kernel_size=(5,5), strides=(1,1) , activation="relu", input_shape=input_shape, name='Conv1')(input_layer)
		x = AveragePooling2D(2, strides=1)(x)
		x = Conv2D(filters=16, kernel_size=(5,5), strides=(1,1) , activation="relu", name='Conv2')(x)
		x = AveragePooling2D(2, strides=1)(x)
		x = Flatten()(x)
