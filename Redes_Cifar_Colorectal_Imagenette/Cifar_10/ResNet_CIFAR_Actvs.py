...
# ResNetOLD = GetNeuralNetworkModel('ResNetOLD',(32,32,3),10, quantization = False, aging_active=False)
#
#
# # In[8]:
#
...
#
# score = ResNet.evaluate(test_ds, verbose=1)
#
# print('Test loss:', score[0])
# print('Test accuracy:', score[1])

#
# score = ResNet.evaluate(train_ds, verbose=1)
#
# print('Test loss:', score[0])
# print('Test accuracy:', score[1])

wgt_dir= ('../Trained_Weights_cifar/ResNet/weights.data')


# df = QuantizationEffect('ResNetOLD',test_ds,wgt_dir,(32,32,3),10,batch_size)
#
#
# quantizacion=pd.DataFrame(df)
#
# quantizacion.to_excel('quantizacion.xlsx', sheet_name='fichero_707', index=False)
# #save_obj(df,'Data/Quantization/ResNet_cifar/Quantization_ResNet_cifar')
#
#
# # score = ResNet.evaluate(train_ds, verbose=1)
# #
# # print('Test loss:', score[0])
# # print('Test accuracy:', score[1])
# print(str()+' Quantización completada ResNet_Cifar: ', datetime.now().strftime("%H:%M:%S"))


# ResNetOLD.load_weights(wgt_dir)
#
#
batch_size = 28
ResNetOLD = GetNeuralNetworkModel('ResNetOLD',(32,32,3),10, quantization = False, aging_active=False)
ResNetOLD.load_weights(wgt_dir).expect_partial()
loss='sparse_categorical_crossentropy'
optimizer = Adam(0.0001)
WeightQuantization(model=ResNetOLD, frac_bits=11, int_bits=4)
ResNetOLD.compile(optimizer=optimizer, loss=loss, metrics='accuracy')
loss, acc = ResNetOLD.evaluate(test_ds)
...
for i in range(1):
    ...
    loss, acc = CheckAccuracyAndLoss('ResNetOLD', test_ds, wgt_dir, output_shape=10, input_shape=(32, 32, 3),
                                     act_frac_size=10, act_int_size=5, wgt_frac_size=14, wgt_int_size=1,
...
# buf_cero.to_excel('ResNetOLD_Cifar_all_v_all_exp.xlsx', sheet_name='fichero_707', index=False)
...
buf_cero.to_excel('Resnet_Cifar_base_F_P.xlsx', sheet_name='fichero_707', index=False)
