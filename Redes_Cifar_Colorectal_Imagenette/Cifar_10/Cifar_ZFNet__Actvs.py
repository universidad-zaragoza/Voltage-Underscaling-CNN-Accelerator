...
ZFNet = GetNeuralNetworkModel('ZFNet',(150,150,3),10, quantization = False, aging_active=activation_aging)
...
wgt_dir= ('../Trained_Weights_cifar/ZFNet/weights.data')


#df = QuantizationEffect('ZFNet',test_ds,wgt_dir,(150,150,3),10,batch_size)
#
#
#quantizacion=pd.DataFrame(df)
#
#quantizacion.to_excel('quantizacion.xlsx', sheet_name='fichero_707', index=False)
#save_obj(df,'Data/Quantization/ZFNet_cifar/Quantization_MobileNet_cifar')

# print(str()+' Quantización completada ZFNet_Cifar: ', datetime.now().strftime("%H:%M:%S"))

ZFNet.load_weights(wgt_dir)

score = ZFNet.evaluate(test_ds, verbose=1)
...
# #
# #
# score = ZFNet.evaluate(train_ds, verbose=1)
# #
...
for i in range(1):
    ...
    loss, acc = CheckAccuracyAndLoss('ZFNet', test_ds, wgt_dir, output_shape=10, input_shape=(150, 150, 3),
...
# buf_cero.to_excel('ZFNet_Cifar_all_v_all_exp.xlsx', sheet_name='fichero_707', index=False)
...
buf_cero.to_excel('ZFNet_Cifar_base_F_P.xlsx', sheet_name='fichero_707', index=False)
print('buf_cero', buf_cero)
