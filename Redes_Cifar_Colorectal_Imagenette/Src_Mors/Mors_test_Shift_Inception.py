...
from Simulation import get_all_outputs,buffer_simulation, save_obj, load_obj
...
diff_sum_FP =[]
activation_aging = [False] * 170
Net = GetNeuralNetworkModel('Inception', (150,150,3), 8, aging_active=activation_aging,
...
F_patch = GetNeuralNetworkModel('Inception', (150,150,3), 8, faulty_addresses=locs, masked_faults=error_mask_new,
...
S_Patch = GetNeuralNetworkModel('Inception', (150,150,3), 8, faulty_addresses=locs, masked_faults=error_mask_new,
                             aging_active=activation_aging, word_size=word_size, frac_size=afrac_size,
                             batch_size=batch_size)
...
from Stats_Shift import WeightQuantization



Net_Shift = GetNeuralNetworkModel('Inception', (150,150,3), 8, faulty_addresses=locs, masked_faults=error_mask_new,
                             aging_active=activation_aging, word_size=word_size, frac_size=afrac_size,
                             batch_size=batch_size)
...
print(test_Mors)
test_Mors.to_excel('MoRS/Modelo3_mas_fallos_col_8_experimentos/estatics/Inception_capa963_ratio_by_layers_by_technique_test_solo LO.xlsx', sheet_name='fichero_707', index=False)
...
#     list_actvs_afect_Shif = []
#     list_actvs_afect_Flip = []
#     if Net.layers[i].__class__.__name__ == 'Lambda':
#         print(Net.layers[i].__class__.__name__)
#         count_lambda = count_lambda+1
#         print('count_lambda',count_lambda)
#
#         iterator = iter(test_ds)
#
#     #while index <= 2:
...
# diff_sum_FP.append(total_F_p)
# capa.append('Total')
# actvs_by_capa.append(len(test_ds)* np.sum(actvs_by_capa))
# actvs_lo_capa.append(len(test_ds)* np.sum(actvs_lo_capa))
# actvs_afect_FP.append(np.sum(actvs_afect_FP))
# actvs_afect_Shif.append(np.sum(actvs_afect_Shif))
#
#
...
# print(test_Mors)
# test_Mors.to_excel('MoRS/Modelo3_mas_fallos_col_8_experimentos/estatics/Inception_ratio_by_layers_Test_shift_all_dataset.xlsx', sheet_name='fichero_707', index=False)
#
#
#
...
# test_Mors.to_excel('MoRS/Modelo3_mas_fallos_col_8_experimentos/estatics/Inception_Test_shift_all_dataset.xlsx', sheet_name='fichero_707', index=False)
#
...
# #
# # Net2 = GetNeuralNetworkModel('Inception', (150,150,3), 8, faulty_addresses=locs, masked_faults=error_mask,
...
# # with pd.ExcelWriter('Inception/métricas/Inception_diff_softmax.xlsx') as writer:
...
# acc_media_comprobacion.columns = ['media_diff','ACC']
# print(buf_same_elemen)
# with pd.ExcelWriter('Inception/métricas/Inception_diff_F_P_16xxx.xlsx') as writer:
#     buf_same_elemen.to_excel(writer, sheet_name='softmax_exp', index=False)
#     acc_media_comprobacion.to_excel(writer, sheet_name='acc_media_comprobacion', index=False)
