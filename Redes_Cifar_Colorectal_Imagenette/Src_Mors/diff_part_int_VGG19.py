...
activation_aging = [False] * 30

# In[4]:



Net1 = GetNeuralNetworkModel('VGG19', (150,150,3), 8, aging_active=activation_aging,
...
dense = Net1.get_layer('dense')
print(dense)
# softmax_layer_out = Net1.get_layer('dense').output
# print('out',softmax_layer_out)

get_softmax_output = K.function([Net1.input, K.learning_phase()], [dense.output])
softmax_output = get_softmax_output([test_ds, 0])[0]
...
acc_media_comprobacion.columns = ['media_diff', 'ACC']
print(buf_same_elemen)
with pd.ExcelWriter('VGG19/métricas/VGG19_diff_softmax_f_p_16xxx.xlsx') as writer:
    buf_same_elemen.to_excel(writer, sheet_name='softmax_exp', index=False)
    acc_media_comprobacion.to_excel(writer, sheet_name='acc_media_comprobacion', index=False)
