...
from tensorflow.python.keras import backend as K
import tensorflow as tf
#from SubClassLayerok import FuncionNotZero
...
def GetNeuralNetworkModel(architecture, input_shape, output_shape, faulty_addresses=[],  masked_faults=[], quantization=True, aging_active=False,
							  shift_apply = False, word_size=None, frac_size=None, batch_size=1):
	print('dentro de get neronal aplicando Shift')
	...
	def Quantization(tensor, active = True):
		...
		return tensor 
	###############################################################################################
    ##FUNCTION NAME: UnderVolting antigua Aging
	...
	### Coloque la funcion Shift aquí, si se quiere aplicar la téctica ha de pasarse shift_applay
	## en True
    ###############################################################################################
	...
	def Shift(tensor, index_list, mod_list, active):
		...
		#print('Aplicando Shift cambio en la recuperacion del signo')
		mask = tf.convert_to_tensor(16384, dtype=tf.int32)
		...
		def ApplyFault(tensor, faults):
			...
			# print('tensor', tensor)
			# print('index_list', faults)
			# print()

			#print('ApplyFault')
			#print('tensorinicial', tensor)
			...
			factor = 2 ** frac_size
			tensor = tf.cast(tensor * factor, dtype=tf.int32)
			#print('tensor despues del cast', tensor)
			signo = tf.math.sign(tensor, name=None)
			#print(tensor)
			tensor = tf.where(tf.less(tensor, 0), -tensor + shift, tensor)
			#print('tensor converción de datos', tensor)
			#original = tensor
			#print('original dentro de tensores', original)
			#test_noZeros = FuncionNotZero(original)
			#original = tf.bitwise.bitwise_and(original, mask)
			#original = tf.bitwise.bitwise_and(original, 0)
			#print('original',original)
			#valores_afectados = tf.math.not_equal(original, 0)
			#print('valores_afectados',valores_afectados)
			#diff_zero_values = tf.math.count_nonzero(valores_afectados)
			#if tf.size(diff_zero_values) > 0:
			...
			#print('tensor aplicar shift', tensor)
			tensor = tf.bitwise.bitwise_and(tensor, faults[:, 0])
			tensor = tf.bitwise.bitwise_or(tensor, faults[:, 1])
			#print('tensor despues de aplicar fallos', tensor)
			tensor = tf.bitwise.right_shift(tensor, bits_shift)
			#print('tensor reuperar el shif', tensor)

			tensor = tensor * signo
			#print('tensor despues recuperar el signo', tensor)
			tensor = tf.where(tf.greater_equal(tensor, shift), shift - tensor, tensor)
			#print('tensor recuperación converción de datos', tensor)

			tensor = tf.cast(tensor / factor, dtype=Ogdtype)

			#print('tensor final despues del cast', tensor)
			return tensor
		...
		###Test del experimento Shif haciendo esta operación a todas las activaciones

	def ShifAllActivations (tensor, index_list, mod_list, active):
		#print('Aplicando Shift')
		mask = tf.convert_to_tensor(16384, dtype=tf.int32)
		#print('dentro de shiftl')
		bits_shift= 1
		...
		#print('original dentro de tensores', original)
		#test_noZeros = FuncionNotZero(original)
		tensor = tf.bitwise.bitwise_and(tensor, mask)
		#original = tf.bitwise.bitwise_and(original, 0)
		#print('original',original)
		#valores_afectados = tf.math.not_equal(original, 0)
		#print('valores_afectados',valores_afectados)
		...
		#tensor = tf.bitwise.left_shift(tensor, bits_shift)
		# affectedValues = tf.gather_nd(tensor, index_list)
		# tensor_with_fails = tf.bitwise.bitwise_and(affectedValues, mod_list[:, 0])
		# tensor_with_fails = tf.bitwise.bitwise_or(tensor_with_fails, mod_list[:, 1])
		# tensor = tf.tensor_scatter_nd_update(tensor, index_list, tensor_with_fails)
		# tensor = tf.bitwise.right_shift(tensor, bits_shift)
		#tensor = tensor * signo
		tensor = tf.where(tf.greater_equal(tensor, shift), shift - tensor, tensor)
		tensor = tf.cast(tensor / factor, dtype=Ogdtype)
	...
	def GenerateAddressList(shape):
		...
		#faults_count_capa.append(faults_count)
		index_list_capa.append(index_list)
		...
		#df_index_list_capa.to_excel('index_list'+ str(architecture)+'.xlsx' ,sheet_name=architecture, index=False)
	...
	def AddCustomLayers(input_layer, include_aging, include_quantization=True, aging_active = []):
		...
		if include_aging:
			...
			x = Lambda(Shift, arguments=aging_arguments)(x)
			#x = Lambda(ShifAllActivations, arguments=aging_arguments)(x)


		# if reverse_shift:
		# 	print('dentro de reverse_shift', reverse_shift)
		# 	reverse_arguments = {'active': tf.identity(reverse_shift)}
		# 	#dims = x.shape.ndims if x.__class__.__name__ == 'KerasTensor' else x.output_shape.ndim
		# 	x = Lambda(ShiftReverse, arguments = reverse_arguments)(x)
	...
	# AI = [2,6,10,12,16,20,22,26,30,34,36,40,44,48,50,54,58,62,64,69,73,77]
		...
		#x = AddCustomLayers(x,reverse_shift = True, include_aging=False, include_quantization=False)
		x = Conv2D(filters=64,kernel_size=(3,3),padding="same")(x)# capa 3
		x = AddCustomLayers(x,include_aging=False) #capa 4
		x = ReLU()(x)# capa  5
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[1],include_quantization=False) # capa  6
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Conv2D(filters=64,kernel_size=(3,3),padding="same")(x)# capa  7
		x = AddCustomLayers(x,include_aging=False)# capa  8
		x = ReLU()(x)  # capa  9
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[2],include_quantization=False) # capa 10
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = MaxPool2D(pool_size=(2,2),strides=(2,2))(x) # capa 11
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[3],include_quantization=False) # capa 12
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Conv2D(filters=128,kernel_size=(3,3),padding="same")(x) # capa 13
		x = AddCustomLayers(x,include_aging=False) # capa 14
		x = ReLU()(x) # capa 15
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[4],include_quantization=False) # capa 16
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Conv2D(filters=128,kernel_size=(3,3),padding="same")(x)# capa 17
		x = AddCustomLayers(x,include_aging=False)# capa 18
		x = ReLU()(x) # capa 19
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[5],include_quantization=False) # capa 20
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = MaxPool2D(pool_size=(2,2),strides=(2,2))(x) # capa 21
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[6],include_quantization=False)# capa 22
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Conv2D(filters=256,kernel_size=(3,3),padding="same")(x)# capa 23
		x = AddCustomLayers(x,include_aging=False)# capa 24
		x = ReLU()(x)# capa 25
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[7],include_quantization=False)# capa 26
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Conv2D(filters=256,kernel_size=(3,3),padding="same")(x)# capa 27
		x = AddCustomLayers(x,include_aging=False)# capa 28
		x = ReLU()(x)# 29
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[8],include_quantization=False)#capa 30
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Conv2D(filters=256,kernel_size=(3,3),padding="same")(x)#capa 31
		x = AddCustomLayers(x,include_aging=False)#capa 32
		x = ReLU()(x)# capa 33
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[9],include_quantization=False)# capa 34
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = MaxPool2D(pool_size=(2,2),strides=(2,2))(x)#capa 35
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[10],include_quantization=False)#capa 36
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Conv2D(filters=512,kernel_size=(3,3),padding="same")(x)#capa 37
		x = AddCustomLayers(x,include_aging=False)#capa 38
		x = ReLU()(x)#capa 39
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[11],include_quantization=False)# capa 40
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Conv2D(filters=512,kernel_size=(3,3),padding="same")(x)# capa 41
		x = AddCustomLayers(x,include_aging=False)# capa 42
		x = ReLU()(x)# capa 43
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[12],include_quantization=False)# capa 44
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Conv2D(filters=512,kernel_size=(3,3),padding="same")(x)# capa 45
		x = AddCustomLayers(x,include_aging=False) # capa 46
		x = ReLU()(x) # capa47
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[13],include_quantization=False)# capa 48
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = MaxPool2D(pool_size=(2,2),strides=(2,2))(x)# capa 49
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[14],include_quantization=False) #capa 50
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Conv2D(filters=512,kernel_size=(3,3),padding="same")(x)#capa 51
		x = AddCustomLayers(x,include_aging=False)#capa 52
		x = ReLU()(x)#capa 53
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[15],include_quantization=False)# capa 54
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Conv2D(filters=512,kernel_size=(3,3),padding="same")(x)#capa 55
		x = AddCustomLayers(x,include_aging=False)# capa 56
		x = ReLU()(x)# capa 57
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[16],include_quantization=False)# capa 58
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Conv2D(filters=512,kernel_size=(3,3),padding="same")(x)# capa 59
		x = AddCustomLayers(x,include_aging=False)#capa 60
		x = ReLU()(x)# capa 61
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[17],include_quantization=False)#62
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = MaxPool2D(pool_size=(2,2),strides=(2,2))(x)#63
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[18],include_quantization=False)#64
		x = Flatten()(x)#65
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Dense(4096)(x)#66
		x = AddCustomLayers(x,include_aging=False)#67
		x = ReLU()(x)#68
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[19],include_quantization=False)#69
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Dense(4096)(x)#70
		x = AddCustomLayers(x,include_aging=False)#71
		x = ReLU()(x)#72
		x = AddCustomLayers(x,include_aging=True,aging_active = aging_active[20],include_quantization=False)#73
		#x = AddCustomLayers(x, reverse_shift=True, include_aging=False, include_quantization=False)
		x = Dense(output_shape)(x)#74
		x = AddCustomLayers(x,include_aging=False)#75
		x = tf.keras.activations.softmax(x)#76
		x = AddCustomLayers(x,include_aging=False)#77
		Net = tf.keras.Model(inputs=input_layer, outputs=x)
		return Net
		...
		print('casi al final del tensor')
