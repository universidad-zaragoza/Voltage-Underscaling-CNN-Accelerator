...
(OrigLoss,OrigAcc) = MobileNet.evaluate(test_set)
print(test_set)
...
Data    = GetReadAndWrites(MobileNet,Indices,817216,100,CNN_gating=False)
stats   = {'Lecturas': Data['Reads'],'Escrituras': Data['Writes']}
Baseline_Acceses   = pd.DataFrame(stats).reset_index(drop=False)
Data    = GetReadAndWrites(MobileNet,Indices,817216,100,CNN_gating=True)
stats    = {'Lecturas': Data['Reads'],'Escrituras': Data['Writes']}
...
CheckAccuracyAndLoss('MobileNet', test_set, wgt_dir, act_frac_size = 11, act_int_size = 4, wgt_frac_size = 14, wgt_int_size = 1,
...
ActivationStats(MobileNet,test_set,11,4,24)
...
for index in range(0,num_of_samples):
    ...
    loss,acc_x   = CheckAccuracyAndLoss('MobileNet', test_dataset, wgt_dir, output_shape=8, input_shape = (224,224,3),
...
save_obj(Accs_x,'Data/Errors/MobileNet/Colorectal Dataset/Accs_x')
#save_obj(Loss,'Data/Errors/MobileNet/Colorectal Dataset/Loss')
...
for i, j in enumerate(ficheros):
    ...
    # loss,acc_w   = CheckAccuracyAndLoss('MobileNet', test_dataset, wgt_dir, output_shape=8, input_shape = (224,224,3),
    #                                         act_frac_size = 11, act_int_size = 4, wgt_frac_size = 14, wgt_int_size = 1,
    #                                         batch_size=testBatchSize, verbose = 0, aging_active = False, weights_faults = True,
    #                                         faulty_addresses = locs, masked_faults = error_mask)
    #
    # loss,acc_a_w   = CheckAccuracyAndLoss('MobileNet', test_dataset, wgt_dir, output_shape=8, input_shape = (224,224,3),
    #                                         act_frac_size = 11, act_int_size = 4, wgt_frac_size = 14, wgt_int_size = 1,
...
for i, j in enumerate(ficheros):
    ...
    loss,acc_1   = CheckAccuracyAndLoss('MobileNet', test_dataset, wgt_dir, output_shape=8, input_shape = (224,224,3),
                                            act_frac_size = 11, act_int_size = 4, wgt_frac_size = 14, wgt_int_size = 1,
                                            batch_size=testBatchSize, verbose = 0, aging_active = True, weights_faults = False,
                                            faulty_addresses = locs, masked_faults = error_mask)

    # loss,acc_w_1   = CheckAccuracyAndLoss('MobileNet', test_dataset, wgt_dir, output_shape=8, input_shape = (224,224,3),
    #                                         act_frac_size = 11, act_int_size = 4, wgt_frac_size = 14, wgt_int_size = 1,
    #                                         batch_size=testBatchSize, verbose = 0, aging_active = False, weights_faults = True,
    #                                         faulty_addresses = locs, masked_faults = error_mask)
    #
    # loss,acc_a_w_1   = CheckAccuracyAndLoss('MobileNet', test_dataset, wgt_dir, output_shape=8, input_shape = (224,224,3),
    #                                         act_frac_size = 11, act_int_size = 4, wgt_frac_size = 14, wgt_int_size = 1,
    #                                         batch_size=testBatchSize, verbose = 0, aging_active = True, weights_faults = True,
    #                                         faulty_addresses = locs, masked_faults = error_mask)
...
print(str()+' operación completada: ', datetime.now().strftime("%H:%M:%S"))
save_file(Accs_1,'Data/Fault Characterization/Accs_buffer2')
...
for i, j in enumerate(ficheros):
    ...
    loss,acc_1_0   = CheckAccuracyAndLoss('MobileNet', test_dataset, wgt_dir, output_shape=8, input_shape = (224,224,3),
                                            act_frac_size = 11, act_int_size = 4, wgt_frac_size = 14, wgt_int_size = 1,
                                            batch_size=testBatchSize, verbose = 0, aging_active = True, weights_faults = False,
                                            faulty_addresses = locs, masked_faults = error_mask)

    # loss,acc_w_1_0   = CheckAccuracyAndLoss('MobileNet', test_dataset, wgt_dir, output_shape=8, input_shape = (224,224,3),
    #                                         act_frac_size = 11, act_int_size = 4, wgt_frac_size = 14, wgt_int_size = 1,
    #                                         batch_size=testBatchSize, verbose = 0, aging_active = False, weights_faults = True,
    #                                         faulty_addresses = locs, masked_faults = error_mask)
    #
    # loss,acc_a_w_1_0   = CheckAccuracyAndLoss('MobileNet', test_dataset, wgt_dir, output_shape=8, input_shape = (224,224,3),
    #                                         act_frac_size = 11, act_int_size = 4, wgt_frac_size = 14, wgt_int_size = 1,
    #                                         batch_size=testBatchSize, verbose = 0, aging_active = True, weights_faults = True,
    #                                         faulty_addresses = locs, masked_faults = error_mask)
...
buf_cero = pd.concat([Acc,Acc_1,Acc_1_0,], axis=1, join='outer')
buf_cero.columns =['Acc_cero', 'Acc_uno', 'Acc_uno_cero']
buf_cero.to_excel('MobileNet_buffer_2_act.xlsx', sheet_name='fichero_707_buffer2', index=False)
