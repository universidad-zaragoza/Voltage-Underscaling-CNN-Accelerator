...
def CheckAccuracyAndLoss(architecture, test_dataset, wgt_dir, act_frac_size, act_int_size, wgt_frac_size, wgt_int_size, input_shape, output_shape,
	...
	#print('debajo de WeightQuantization')
	...
	if architecture == 'PilotNet':
		...
		Relu_out = activations[4]
		#print('cantidad de ekementos', len(Relu_inp))
		#print(Relu_inp.shape)
		#print(Relu_inp)
		#print(type(Relu_inp))
		#print('cantidad de ekementos', Relu_inp.size)
...
## Esta se ha restructurado para hacer el análisis por contando los accesos de lecturas solo 1 vez para cada indice según va pasando por las capas
...
###Para las escrituras, esto no tiene problema porque es simple y lo da bien#############################################3
		np.put(data_read_write['Writes'], range(data_read_write['offset'], data_read_write['offset'] + out_size), 1 + actual_values, mode='wrap')
		print(' capa, tamaño', layer.__class__.__name__,out_size)
		print('suma de las escrituras' ,actual_values)
		...
		print('escrituras bloque de 16', write_block)
		array_read_block_newForm.append(write_block)
		#array_write_by_block.append(write_block)
		get_next_address(out_size)
	def Buffer_Reading(layer):
		print('capas de leturas')
		...
		if layer.__class__.__name__ in ['MaxPooling2D','GlobalAveragePooling2D','MaxPooling1D','AveragePooling2D','BatchNormalization']:
			...
			print('layer.input_shape[1:])', layer.input_shape[1:])
...
####Utilizo mi variable para guardar los valores nuevos que deseo en este caso me va contando la cantidad de veces que se lee un index ########
			...
			array_read_block_newForm.append(read_block)
			# write_read_layers.append(layer.__class__.__name__)
			#array_read_by_block.append(read_block)
			write_read_layers.append(layer.__class__.__name__+  str('_') +  str('read'))
			print('shape de estas capas',layer.input_shape)
			print(' capa, tamaño', layer.__class__.__name__, in_size)
			print('suma de las lecturas', actual_values)
			print('lecturas bloque de 16', in_size/ 16)
##Si quiero el análisis por bloques debo limpiar esta variable para qu eno me cuente de más
			#np.put(data_read_write['Reads'], range(data_read_write['offset'], data_read_write['offset'] + in_size),   0, mode='wrap')
#####Aquí es donde creo que está el tema ###########################################3
			...
			read_block_newForm=( input_height*input_width*input_channels * np.ceil(input_channels/PE_columns))/16
			array_read_block_newForm.append(read_block_newForm)
			print(' capa, tamaño', layer.__class__.__name__, (input_height*input_width*input_channels))
			print('suma de las lecturas no las calculo aquí, debo analizar ')
			print('lecturas bloque de 16', read_block_newForm)
...
#del kernel , la cantidad de canales  así como las dimenciones de la imagen
			...
			if layer.__class__.__name__ in ['Conv2D']:
				print('Conv2D', layer.__class__.__name__)
...
##Si quiero el análisis por bloques debo limpiar esta variable para qu eno me cuente de más
			...
			print('layer.input_shape', layer.__class__.__name__)
			print('filtros', layer.filters)
			#Convolution 1D
			(input_width,input_channels) = layer.input_shape[1:]
			read_block_newForm = (input_width * input_channels * np.ceil(input_channels / PE_columns)) / 16
			array_read_block_newForm.append(read_block_newForm)
			print(' capa, tamaño', layer.__class__.__name__, ( input_width * input_channels))
			print('suma de las lecturas no las calculo aquí, debo analizar ')
			print('lecturas bloque de 16', read_block_newForm)
			padding = layer.padding
			#Create Activation Map Reads Count
...
#############La fórmula varía porque el shape==3
			...
			if layer.__class__.__name__ in ['Conv1D']:
				print('Conv1D', layer.__class__.__name__)
				input_map = input_map*(np.ceil(output_channels/PE_columns).astype(np.int64))
...
##Si quiero el análisis por bloques debo limpiar esta variable para qu eno me cuente de más
			#np.put(data_read_write['Reads'],  range(data_read_write['offset'], data_read_write['offset'] + input_map.size),  0, mode='wrap')
			#print('data_read_write :Reads establecido en 0', data_read_write['Reads'])
		else:
			...
			print('lecturas bloque de 16', read_by_block)
		...
		#Aquí mantuve la formula original porque sonsidero qu ees así para estas capas
			#read_block_newForm = (input_size * np.ceil(output_size / (PE_columns * PE_rows))) / 16
			#array_read_block_newForm.append(read_block_newForm)
			...
			#array_read_by_block.append(read_block)
			write_read_layers.append(layer.__class__.__name__ +  str('_') +  str('read'))
			array_read_block_newForm.append(read_by_block)
##Si quiero el análisis por bloques debo limpiar esta variable para qu eno me cuente de más
			#np.put(data_read_write['Reads'], range(data_read_write['offset'], data_read_write['offset'] + input_size),  0, mode='wrap')
			#print('data_read_write_Reads establecido en 0', data_read_write['Reads'])
	...
	# df_read_block = pd.DataFrame(array_read_by_block)
	# df_write_layers = pd.DataFrame(write_layers)
	# df_write_block = pd.DataFrame(array_write_by_block)
	df_write_read_layers = pd.DataFrame(write_read_layers)
	# df_write_read_block= pd.DataFrame(array_write_read_block)
	df_array_read_block_newForm=pd.DataFrame(array_read_block_newForm)
	df_Block = pd.concat([df_write_read_layers, df_array_read_block_newForm], axis=1, join='outer')
	df_Block.columns = ['Capa', 'Read_write_block']
	df_Block.style.applymap(lambda x: [ 'color: red' if df_Block['write'] in x else 'color: black'], axis=None)
	print(df_Block)
	#df_Block.to_excel('read_write_blocks_test' + str(network_name) + '.xlsx', sheet_name='fichero_707', index=False)


	# df_w_r_Block = pd.concat([df_read_layers, df_read_block,df_write_layers,df_write_block ], axis=1, join='outer')
	# df_w_r_Block.columns = ['capa_red', 'Read_block', 'capa_write', 'write_block']
	# # print('df_read_Block', df_read_Block)
	# df_w_r_Block.to_excel('read_write_blocks_Mors' + str(network_name) + '.xlsx', sheet_name='fichero_707', index=False)
	#df_write_Block_new_form = pd.concat([df_write_read_layers, df_write_read_block], axis=1, join='outer')
	#df_write_Block_new_form.columns = ['Capa', 'Write_read_block']
	#df_w_r_Block.to_excel('write_read' + str(network_name) + '.xlsx', sheet_name='fichero_707', index=False)
	#df_write_Block_new_form.to_excel('read_and_write_newForm_PE_16' + str(network_name) + '.xlsx', sheet_name='fichero_707', index=False)
