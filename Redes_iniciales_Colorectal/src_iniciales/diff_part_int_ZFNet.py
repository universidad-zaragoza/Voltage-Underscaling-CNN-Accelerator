...
# with pd.ExcelWriter('ZFNet/métricas/ZFNet_diff_softmax_imagenes.xlsx') as writer:
...
acc_media_comprobacion.columns = ['media_diff','ACC']
print(buf_same_elemen)
with pd.ExcelWriter('ZFNet/métricas/ZFNet_diff_softmax_f_p_vbw_16xx.xlsx') as writer:
    buf_same_elemen.to_excel(writer, sheet_name='softmax_exp', index=False)
    acc_media_comprobacion.to_excel(writer, sheet_name='acc_media_comprobacion', index=False)
...
# list_ciclo = []
#
# with pd.ExcelWriter('ZFNet/ZFNet_softmax_base.xlsx') as writer:
