...
def GetNeuralNetworkModel(architecture, input_shape, output_shape, faulty_addresses = [],
                         	 masked_faults = [], quantization = True, aging_active = [], neighbor_active=True, 
    ...
    ###############################################################################################
	def Aging(tensor, index_list, mod_list,index_list_neig, active=False, neighbor_active=False):
		#print('neighbor_active dentro de Aging',active_neighbor)
		#print('neighbor_active dentro de Aging',active)
		...
		def ApplyFault(tensor,faults):
			...
			#print('tensor despyes del cast',tensor)
			tensor  = tf.where(tf.less(tensor, 0), -tensor + shift , tensor )
			#print('tensor despues del less where',tensor)
			tensor  = tf.bitwise.bitwise_and(tensor,faults[:,0])
			#print('tensor despues del bitwisw o',tensor)
			tensor  = tf.bitwise.bitwise_or(tensor,faults[:,1])
			#print('tensor despues del bitwisw 1',tensor)
			tensor  = tf.where(tf.greater_equal(tensor,shift), shift-tensor , tensor )
			#print('tensor despues de equal',tensor)
			tensor  = tf.cast(tensor/factor,dtype = Ogdtype)	
			#print('tensor final',tensor)	
			return tensor
		if active:
			print('Vecinos')
			newValuesantes = tf.gather_nd(tensor, index_list)			
			newValues = tf.gather_nd(tensor, index_list_neig)
			print('newValuesvecino', newValues)
			print('newValues antes',newValuesantes)
			tensor = tf.tensor_scatter_nd_update(tensor, index_list, newValues)	
			newValuesdesp = tf.gather_nd(tensor, index_list)
			print('nuevo newValuesdesp', newValuesdesp)
			cantidad_valores_iguales=tf.equal(newValuesantes, newValuesdesp)
			print('cantidad_valores_iguales', cantidad_valores_iguales)
			#numpy_tensor=tensor.numpy()
			#print('tensor_resultante', numpy_tensor)
		...
		""" def neighbor(tensor, index_list_neig, index_list, neighbor_active):
			newValues = tf.gather_nd(tensor, index_list_neig)
			tensor = tf.tensor_scatter_nd_update(tensor, index_list, newValues)
			return tensor """
		...
		# if neighbor_active:
		# 	#index_list_np=index_list.numpy()
		# 	#print('index_list antes',index_list_np)
		# 	#print(type(index_list_np))
		# 	index_vecino=[]
		#
		# 	index_vecino=[x[1:]+1 for x in index_list_np]
		# 	#print('index_vecino ',index_vecino)
		# 	#tf.convert_to_tensor(index_vecino)
		# 	#affectedValues = tf.gather_nd(tensor,index_list)
		# 	newValues = tf.gather_nd(tensor,index_vecino)
		# 	tensor = tf.tensor_scatter_nd_update(tensor, index_list, newValues)
	...
	def GenerateAddressList(shape):
			...
			for index in range(batch_size):
				for address,mask in zip(faulty_addresses,masked_faults):
					if address < shape[0]-1:
						...
						index_list_neig.append([index,address+1])
			...
			for index in range(batch_size):
				for address,mask in zip(faulty_addresses,masked_faults):
					if address < shape[0]*shape[1] - 1:
						...
						index_list_neig.append([index,Ch1+1,Ch2+1])
						mod_list.append(DecodeMask(mask))
						error_zero.append(collections.Counter(mask)['0'])
						error_uno.append(collections.Counter(mask)['1'])
			...
			for index in range(batch_size):
				...
				for address,mask in zip(faulty_addresses,masked_faults):
					if address < shape[0]*shape[1]*shape[2] - 1:
						...
						index_list_neig.append([index,row_n,col_n,actMap])
		...
		#df_error.to_excel('resul_'+ str(architecture)+'.xlsx' ,sheet_name=architecture, index=False)
		...
		#      print('dentro de withwriter')
		#print('index_list fuera del ciclo', len(index_list))
		#print('errores totales', total_error)
		#print('errores capa', total_error_capa)
		#print('index_list_capa', index_list_capa)

		return tf.convert_to_tensor(index_list),tf.convert_to_tensor(mod_list), tf.convert_to_tensor(index_list_neig),faults_count,
	...
	def AddCustomLayers(input_layer, include_aging, include_quantization=True, aging_active= []):
		print('aging_active dentro de addcustomLayer',aging_active)
		#print('neighbor_active dentro de addcustomLayer',neighbor_active)
		# print('include_aging',include_aging)
		x = input_layer
		...
		if include_aging:
			...
			index_list,mod_list,index_list_neig,faults_count = GenerateAddressList(shape=x.shape[1:])
	##se crea un tensor que representa el valor de esa variable en el momento en que se llama
	        
			aging_arguments = {'index_list' : tf.identity(index_list),
			                   'index_list_neig': tf.identity(index_list_neig),
