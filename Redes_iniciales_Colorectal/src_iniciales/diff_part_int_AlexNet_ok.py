...
def diferenc_intpart(outputs1, outputs2, ciclo, acc_list):
    ...
    for index in range(0, len(outputs2)):
        if index == write_layer[ciclo]:
            ...
            diff_nets =((tf.math.subtract(outputs1[index], outputs2[index])))
            print('si los valores son iguales ha de dar 0',np.sum(diff_nets))
            ...
            if  outputs1[index] == outputs2[index]:
                print('salidas iguales')
...
def SofmaxDiffElements(outputs1, outputs2, acc_list):
    ...
    for index in range(0, len(outputs2)):
        ...
        if index == 52:
            ...
            size_output = a.size
            output_true = np.sum(a)
            print('cantidad de elementos iguales de ambas salidas', output_true)
            numero.append(index)
            capa.append(Net2.layers[index].__class__.__name__)
            list_output_true.append(output_true)
            list_size_output.append(size_output)
...
F = GetNeuralNetworkModel('AlexNet', (227, 227, 3), 8, faulty_addresses=locs, masked_faults=error_mask,
                             aging_active=activation_aging, word_size=word_size, frac_size=afrac_size,
                             batch_size=testBatchSize)
F.load_weights(wgt_dir).expect_partial()
loss = tf.keras.losses.CategoricalCrossentropy()
optimizer = tf.keras.optimizers.Adam(learning_rate=1e-3)
WeightQuantization(model=F, frac_bits=wfrac_size, int_bits=wint_size)
F.compile(optimizer=optimizer, loss=loss, metrics='accuracy')
loss, acc = F.evaluate(test_dataset)
acc_list.append(acc)
# list_ciclo.append(i)
# index = 1
# while index <= len(test_dataset):
...
while index <= len(test_dataset):
    ...
    print('index+++++++++++++++++++++++++++', index)
    # print('imagen',index, image)
    outputs1 = get_all_outputs(F, image)
    # salidas del modelo sin fallas para la primer imagen del dataset de prueba
    outputs2 = get_all_outputs(Net2, image)
    # salidas del modelo con fallas para la primer imagen del dataset de prueba
    diff_nets_sum = SofmaxDiffElements(outputs1, outputs2, acc_list)
...
# with pd.ExcelWriter('AlexNet/métricas/AlexNet_diff_softmax_f_p_vbw_16xx_prueba.xlsx') as writer:
