...
Net1 = GetNeuralNetworkModel('SqueezeNet', (224,224,3), 8, aging_active=activation_aging,
...
acc_media_comprobacion.columns = ['media_diff','ACC']
print(buf_same_elemen)
with pd.ExcelWriter('SqueezeNet/métricas/SqueezeNet_diff_softmax_f_p_vbw_16xx_prueba.xlsx') as writer:
    buf_same_elemen.to_excel(writer, sheet_name='softmax_exp', index=False)
    acc_media_comprobacion.to_excel(writer, sheet_name='acc_media_comprobacion', index=False)
...
# list_ciclo = []
#
# with pd.ExcelWriter('SqueezeNet/SqueezeNet_ratio_part_int_base.xlsx') as writer:
