...
DF_Funcion = pd.DataFrame(['Base', 'ECC', 'FlipPatch','ScratchPad','Shift'])

DF_Funcion.to_excel('MoRS/Modelo3_col_8_'+ str(vol)+'/Analisis_Resultados/estatics/tesis/test_direccion.xlsx', sheet_name='fichero_707', index=False)
...
for j , vol  in enumerate(voltajes):
    ...
    for inc in range(8,10):
        ...
        error_mask_H_L_O, locs_H_L_O, index_locs_VBW = VeryBadWords(error_mask, locs)
        error_mask_les_VBW, locs_less_VBW = DeleteTercioRamdom(error_mask, locs, index_locs_VBW)
        ...
        Funcion = [Base, ECC, FlipPatchBetter, ScratchPad, Shift]
        #Funcion = [ FlipPatchBetter]
        for i in range(len(Funcion)):
            print('Ciclo interno', i)
            print('funcion', Funcion[i])
            inicio = time.time()

            # WordType(error_mask,locs)

            error_mask_new, locs,locs_modif,word_change = Funcion[i](error_mask_les_VBW, locs)
        ...
        Acc_all_exp = pd.concat(   [Df_change_words, DF_Funcion, Acc_AlexNet, Acc_DenseNet, Acc_MobileNet, Acc_SqueezeNet, Acc_VGG16, Acc_ZFNet],
            axis=1, join='outer')
        print(Acc_all_exp)
        Acc_all_exp.columns = ['words_changej', 'Técnica', 'AlexNet', 'DenseNet', 'MobileNet', 'SqueezeNet', 'VGG16',   'ZFNet']
        Acc_all_exp.to_excel(
            'MoRS/Modelo3_col_8_' + str(vol) + '/Analisis_Resultados/estatics/tesis/ACC_all_Experiment_modelo_' + str(
        ...
        # Acc_all_exp = pd.concat([Df_change_words, DF_Funcion, Acc_AlexNet, Acc_MobileNet],  axis=1, join='outer')
        # print(Acc_all_exp)
        # Acc_all_exp.columns = ['words_changej', 'Técnica', 'AlexNet',  'MobileNet']
        # Acc_all_exp = pd.concat([pd.DataFrame([None] * Acc_all_exp.shape[1], index=Acc_all_exp.columns).T, Acc_all_exp],
        #                         ignore_index=True)
        # Acc_all_exp.loc[0] = [''] + ['original'] + original
        # print(Acc_all_exp)
        # Acc_all_exp.to_excel( 'MoRS/Modelo3_col_8_' + str(vol) + '/Analisis_Resultados/estatics/ACC_all_Experiment_modelo_test_tesis' + str(
        #         vol) + '_' + str(inc) + '.xlsx', sheet_name='fichero_707', index=False)
